  ...
  controller(id, nav)
  {
    ...
    s.action = (strAction, strNode) =>
    {
      if (s.bDeveloper)
      {
        s.bDisabled = true;
        s.u();
        let request = {Interface: 'feedback', 'Function': 'action', Request: {Action: strAction, Node: strNode}};
    ...
    s.init = () =>
    {
      ...
      c.wsRequest('radial', request).then((response) =>
      {
        ...
        if (c.wsResponse(response, error))
        {
          ...
          {
            s.nodes[nodes[i]] = {};
            let request = {Interface: 'feedback', 'Function': 'status', Request: {Node: nodes[i]}};
            if (nodes[i] != strNode)
            {
