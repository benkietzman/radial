...
bool Db::dbCentralUserReminderUpdate(Json *i, Json *o, string &id, string &q, string &e)
{
  ...
  if (dep({"id",}, i, e))
  {
    bool f = true;
    stringstream qs;
    qs << "update person_reminder set" << u({"alert", "chat", "cron", "description", "email", "live", "person_id", "sched", "text", "title"}, i, f) << " where id = " << v(i->m["id"]->v);
    b = dbu("central", qs, q, e);
  }
...
bool Db::dbCentralUsers(Json *i, Json *o, string &id, string &q, string &e)
{
  ...
  else
  {
    ...
    qs << "alert_remote_auth_user, alert_remote_proxy, ";
    if (!m_strAesSecret.empty())
    {
