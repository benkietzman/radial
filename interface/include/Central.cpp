...
Central::Central(string strPrefix, int argc, char **argv, void (*pCallback)(string, const string, const bool)) : Interface(strPrefix, "central", argc, argv, pCallback)
{
  ...
  m_functions["monitorConfig"] = &Central::monitorConfig;
  m_functions["monitorData"] = &Central::monitorData;
  m_functions["monitorProcess"] = &Central::monitorProcess;
  m_functions["monitorSystem"] = &Central::monitorSystem;
  m_functions["monitorUpdate"] = &Central::monitorUpdate;
  ...
  if (dep({"id"}, i, e))
  {
    ...
    if (db("dbCentralApplicationIssues", i, r, e))
    {
      if (!r.empty())
      {
        ...
        if (i->val({"comments"}) == "1" && !j->empty({"id"}))
        {
          radialUser a;
          userInit(d, a);
        ...
        if (applicationIssue(a, e) && !a.p->m["o"]->empty({"application_id"}))
        {
          ...
          if (application(c, e) && !c.p->m["o"]->empty({"id"}) && !c.p->m["o"]->empty({"name"}))
          {
            ...
            if (applicationUsersByApplicationID(f, e))
            {
              ...
              if (a.p->m["o"]->exist({"comments"}) && !a.p->m["o"]->m["comments"]->l.empty())
              {
            ...
            if (applicationUsersByApplicationID(f, e))
            {
              ...
              for (auto &contact : f.p->m["o"]->l)
              {
                if (!contact->empty({"user_id"}) && !contact->empty({"userid"}) && contact->val({"notify", "value"}) == "1")
                {
                  ...
                  else if (contact->val({"type", "type"}) == "Backup Developer")
                  {
                    if (contact->val({"notify", "value"}) == "1")
                    {
                  ...
                  else if (contact->val({"notify", "value"}) == "1")
                  {
              ...
              userDeinit(g);
              if (bDeveloper && i->val({"notifyDependents", "value"}) == "1")
              {
                radialUser h;
                userInit(d, h);
                h.p->m["i"]->i("application_id", c.p->m["o"]->m["id"]->v);
                h.p->m["i"]->i("contacts", "1", 'n');
                if (dependentsByApplicationID(h, e) && h.p->m["o"]->exist({"dependents"}))
                {
            ...
            if (groupUsersByGroupID(f, e))
            {
              ...
              for (auto &contact : f.p->m["o"]->l)
              {
                if (!contact->empty({"user_id"}) && !contact->empty({"userid"}) && contact->val({"notify", "value"}) == "1")
                {
                  ...
                  else if (contact->val({"type", "type"}) == "Backup Owner")
                  {
                    if (contact->val({"notify", "value"}) == "1")
                    {
                      o->m[contact->m["userid"]->v]->i("backup", "1", 1);
...
bool Central::loginPasskeyRegister(radialUser &d, string &e)
{
  ...
  ssHtml << "It is recommended to register a passkey.  You can do that once authenticated by clicking the Profile menu in <a href=\"https://" << m_strServer << "/central\" target=\"_blank\">Central</a> and clicking the Passkeys button in the Security Profile section.  Registering a passkey enhances your online security and simplifies the login process by eliminating the need to remember complex passwords.  It allows you to authenticate using biometric methods or a PIN, making it faster and more intuitive.";
  d.p->i("o", ssHtml.str());
...
// }}}
// {{{ monitorConfig()
bool Central::monitorConfig(radialUser &d, string &e)
{
  bool b = false;
  Json *i = d.p->m["i"], *o = d.p->m["o"];
  ...
  if (dep({"server"}, i, e))
  {
    if (i->exist({"system"}))
    {
      if (auth(d.r, e))
      {
        ...
        ptData->m["system"] = new Json(i->m["system"]);
        if (i->exist({"processes"}))
        {
          ptData->m["processes"] = new Json(i->m["processes"]);
        }
        if (storageAdd({"central", "monitor", "servers", i->m["server"]->v, "data"}, ptData, e))
        {
          b = true;
        }
        delete ptData;
...
// }}}
// {{{ monitorProcess()
bool Central::monitorProcess(radialUser &d, string &e)
{
  bool b = false;
  Json *i = d.p->m["i"], *o = d.p->m["o"];
...
// }}}
// {{{ monitorSystem()
bool Central::monitorSystem(radialUser &d, string &e)
{
  bool b = false;
  Json *i = d.p->m["i"], *o = d.p->m["o"];
...
void Central::schedule(string strPrefix)
{
  ...
  string strError, strQuery, strValue;
  stringstream ssMessage, ssQuery;
  time_t CMonitor = 0, CNow, CReminders = 0, CStatistics = 0, CStatus, CWorkload = 0;
  Json *ptJson;
    ...
    if (isMasterSettled() && isMaster())
    {
      ...
      if ((CNow - CMonitor) > 30)
      {
        ...
        if (storageRetrieve({"central", "monitor", "servers"}, ptJson, strError))
        {
          ...
          for (auto &server : ptJson->m)
          {
                      ...
                      if (ptData->exist({"system"}))
                      {
                        ...
                        if (ptData->exist({"processes"}))
                        {
                          for (auto &process : ptData->m["processes"]->m)
                          {
                            ...
                            {
                              if (ptConfig->m["processes"]->exist({process.first}))
                              {
                                stringstream ssAlarmsProcess;
                                Json *ptConfigProcess = ptConfig->m["processes"]->m[process.first];
                                if (!ptAlarms->m["processes"]->exist({process.first}))
                                {
                                  ptAlarms->m["processes"]->m[process.first] = new Json;
                                }
                                if (!ptAlarms->m["processes"]->m[process.first]->exist({"alarms"}))
                                {
                                ...
                                {
                                  if (ptDataProcess->exist({"owners"}) && !ptConfigProcess->empty({"owner"}))
                                  {
                                    bool bFound = false;
                                ...
                                if (ptAlarms->m["processes"]->m[process.first]->m["alarms"]->v != ssAlarmsProcess.str())
                                {
                                  ...
                                  if (ptAlarms->m["processes"]->m[process.first]->exist({"sent"}))
                                  {
            ...
            {
              ptAlarms->i("system", ssAlarmsSystem.str());
      ...
      if (m_bLoadReminders)
      {
        ...
        if (db("dbCentralUserReminders", ptReminder, getReminder, strQuery, strError))
        {
          ...
          storageRemove({"central", "loadReminders"}, strError);
        }
        else
        {
          ssMessage.str("");
      ...
      // }}}
      // {{{ workload
      if (CWorkload == 0 && !cron(CWorkload, "0 4 * * 1", strError))
      {
        ssMessage.str("");
            ...
            if (serverUsersByServerID(f, e))
            {
              ...
              for (auto &contact : f.p->m["o"]->l)
              {
                if (!contact->empty({"user_id"}) && !contact->empty({"userid"}) && contact->val({"notify", "value"}) == "1" && !contact->empty({"email"}))
                {
                  ...
                  else if (contact->val({"type", "type"}) == "Backup Admin")
                  {
                    if (contact->val({"notify", "value"}) == "1")
                    {
                      o->m[contact->m["userid"]->v]->i("backup", "1", 1);
