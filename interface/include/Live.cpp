...
timespec start, stop; clock_gettime(CLOCK_REALTIME, &start);
    hub("websocket", ptSubJson, bWait);
clock_gettime(CLOCK_REALTIME, &stop); log((string)"Interface::live()->Interface::hub(localhost,link) " + to_string(((stop.tv_sec - start.tv_sec) * 1000) + ((stop.tv_nsec - start.tv_nsec) / 1000000)));
    delete ptSubJson;
  }
  m_mutex.unlock();
  if (!bFound)
...
timespec start, stop; clock_gettime(CLOCK_REALTIME, &start);
        hub("link", ptDeepJson, bWait);
clock_gettime(CLOCK_REALTIME, &stop); log((string)"Interface::live()->Interface::hub(" + req.first + (string)",link) " + to_string(((stop.tv_sec - start.tv_sec) * 1000) + ((stop.tv_nsec - start.tv_nsec) / 1000000)));
        delete ptDeepJson;
      }
      delete req.second;
    }
