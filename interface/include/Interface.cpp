...
void Interface::callbackPool()
{
  ...
  while (!shutdown())
  {
    ...
    m_mutexShare.lock();
    while (!shutdown() && !m_callbacks.empty())
    {
    ...
    if (!workers.empty())
    {
      ...
      while (!bDone)
      {
        ...
        {
          if ((*i)->fdWorker[0] == -1 || ((*i)->callbacks.empty() && (CTime - (*i)->CTime) > 10))
          {
            workerIter = i;
          }
        }
        if (workerIter != workers.end())
        {
          ...
          while ((*workerIter)->fdWorker[0] != -1)
          {
            msleep(10);
          }
          while (!(*workerIter)->callbacks.empty())
          {
            delete (*workerIter)->callbacks.front();
            (*workerIter)->callbacks.pop();
          }
          delete (*workerIter);
  ...
  for (auto &worker : workers)
  {
    ...
    while (worker->fdWorker[0] != -1)
    {
      msleep(10);
    }
    while (!worker->callbacks.empty())
    {
      delete worker->callbacks.front();
      worker->callbacks.pop();
    }
    delete worker;
    threadDecrement();
  }
...
void Interface::process(string strPrefix)
{
  ...
  if ((nReturn = pipe(m_fdResponse)) == 0)
  {
    ...
    time_t CBroadcast, CMaster[2], CThroughput, CTime, unBroadcastSleep = 10;
    ...
    while (!bExit)
    {
      ...
      if (m_pAutoModeCallback != NULL)
      {
        if (!m_bMasterSettled && !m_strMaster.empty() && (CTime - CMaster[1]) > 7)
        {
